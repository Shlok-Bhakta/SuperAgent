...
TIMEOUT = 500 # time for each move
...
def main(swapPlayers):
    ...
    if swapPlayers:
        ...
        # judge = Judge("http://127.0.0.1:5008", "http://10.245.190.221:5009")
    else:
        judge = Judge("http://127.0.0.1:5008", "http://127.0.0.1:5009")
        # judge = Judge("http://10.245.190.221:5009", "http://127.0.0.1:5008")
    ...
    print(f"Player 1: {judge.p1_agent.agent_name} ({judge.p1_agent.participant}:{5008 if swapPlayers else 5009})")
    print(f"Player 2: {judge.p2_agent.agent_name} ({judge.p2_agent.participant}:{5009 if swapPlayers else 5008})")
        ...
        if not first_attempt:
            ...
            if not second_attempt:
                ...
                if current_random_moves > 0:
                    ...
                    print(judge.game.display_board())
                    f.write(str((int("".join("".join("1" if i == 1 else "0" for i in r) for r in judge.game.board), 2))))
                    judge.handle_move(judge.game, move)                    # tag that it was random
                    judge.game_str += 'r'
...
if __name__ == "__main__":
    ...
    Ties = 0
    while (games > 0):
        swapPlayers = (games % 2) == 0
        result = main(swapPlayers)
        if (swapPlayers):
            if(result == 1):
                p1 +=1
            elif(result == -1):
            ...
            if(result == 1):
                ...
                Ties+=1
        games-=1
        print(f"GAME INFO! P1: {p1} | P2: {p2} | Ties: {Ties}")
